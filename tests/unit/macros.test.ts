  ...
  describe("macro()", () => {
    ...
    it("should throw error when registering duplicate macro", () => {
      ...
      expect(() => {
        ...
        "Macro duplicate already defined! You are not allowed to overwrite it."
      )
    })
  })
  ...
  describe("from()", () => {
    it("should apply a registered macro", () => {
      config.macro("create-jobs", (cfg, args: { jobs: string[] }) => {
        for (const jobName of args.jobs) {
      ...
      const j1 = jobs.job1
      if (!j1) {
        throw new Error("Expected job1 to be present")
      }
      expect(j1.script).toStrictEqual(["echo job1"])
    ...
    it("should pass arguments correctly to macro", () => {
      config.macro(
        "with-args",
        (cfg, args: { stage: string; tags: string[] }) => {
          cfg.job("test-job", {
            stage: args.stage,
            tags: args.tags,
            script: ["test"],
      ...
      if (!tj) {
        throw new Error("Expected test-job to be present")
      }

      expect(tj.stage).toBe("test")
      expect(tj.tags).toStrictEqual(["docker", "linux"])
    })
  })
})
