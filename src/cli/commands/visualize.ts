...
import type { VisualizationFormat } from "../../resolution"
import { visualizeYaml } from "../../resolution"
...
export default function visualizeCommand() {
  ...
  program
    ...
    .option("-f, --format <format>", "Output format: mermaid, ascii, table, all", "all")
...
`,
    ...
    .action(async (input, options) => {
      ...
      if (!["mermaid", "ascii", "table", "all"].includes(format)) {
        ...
        console.error(`Invalid format: ${format}. Must be one of: mermaid, ascii, table, all`)
        process.exit(1)
      }
